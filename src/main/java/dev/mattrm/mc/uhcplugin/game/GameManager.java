...
import dev.mattrm.mc.uhcplugin.settings.enums.TimeCycle;
import dev.mattrm.mc.uhcplugin.settings.enums.WeatherCycle;
import dev.mattrm.mc.uhcplugin.util.ChatUtils;
import org.bukkit.*;
...
public class GameManager extends Service {
    ...
    protected void setupService() {
        this.plugin.getServer().getPluginManager().registerEvents(new GameEventsListener(), this.plugin);
        UHCScoreboards.setup(this.plugin);
    }
    ...
    public void enterSetupPhase() {
        ...
        WorldSyncService.getInstance()
            ...
            .setGameRuleValue("showDeathMessages", "true");
    ...
    public void enterPregamePhase() {
        ...
        WorldSyncService.getInstance()
            .setWorldBorderCenter(0.5, 0.5)
            .setWorldBorderSize(initialBorderSize)
            .setWorldBorderWarningTime(180)
            .setWorldBorderWarningDistance(10)
            .setWorldBorderDamageBuffer(5)
            .setWorldBorderDamageAmount(0.2);
        this.worldborderSize.setAndNotify(initialBorderSize);

        // Setup gamerules
        WorldSyncService.getInstance()
            .setGameRuleValue("doDaylightCycle", String.valueOf(SettingsManager.getInstance().timeCycle().get() == TimeCycle.NORMAL))
            .setGameRuleValue("doWeatherCycle", String.valueOf(SettingsManager.getInstance().weatherCycle().get() == WeatherCycle.NORMAL));
        ...
        if (SettingsManager.getInstance().teamGame().get() == TeamStatus.INDIVIDUAL_GAME || !SettingsManager.getInstance().teamsSpawnTogether().get()) {
            ...
            for (int i = 0; i < numTeams; i++) {
                ...
                int j = i;
                TeamService.getInstance().getOnlineTeamMembers(teams.get(i).getId()).forEach(player -> {
                    int x, z;
                    Location loc;
                    do {
            ...
            this.competitors = competitors.stream().map(Player::getUniqueId).distinct().collect(Collectors.toList());
            this.aliveCompetitors = new ArrayList<>(this.competitors);
            this.aliveTeams = teams.stream().map(GameTeam::getId).collect(Collectors.toMap(t -> t, t -> TeamService.getInstance().getOnlineTeamMembers(t).stream().map(Player::getUniqueId).collect(Collectors.toCollection(ArrayList::new))));
        }
        ...
        // Wait for ready up
        ReadyUpService.getInstance().setPrefix(ChatUtils.PREFIX);
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                if (this.count > 0) {
                    ...
                    WorldSyncService.getInstance().setWorldBorderSize(worldborderTransitions.get(0), times.get(0) * 60);
    ...
    public void transitionToWorldBorderStopped() {
        ...
        SettingsManager.getInstance().timeCycle().set(TimeCycle.DAY_ONLY);
        int parlayTime = SettingsManager.getInstance().suddenDeathParlayTime().get();
        ...
        this.timer.addTempHook(new Consumer<Integer>() {
            ...
            public void accept(Integer taskId) {
                ...
                if (adjustedTime <= 0) {
                    ...
                    WorldSyncService.getInstance().setWorldBorderSize(15, 52);
                    timer.removeHook(taskId);
                } else if (adjustedTime <= minAlert * 60L) {
    ...
    public void handleWorldBorderTransition() {
        ...
        WorldSyncService.getInstance().setWorldBorderSize(this.worldborderTransitions.get(1), this.worldborderTimes.get(0) * 60);
    ...
    public List<Player> getInitialCompetitors() {
        if (SettingsManager.getInstance().teamGame().get() == TeamStatus.TEAM_GAME) {
            return this.getInitialTeams().stream()
                .flatMap(team -> TeamService.getInstance().getOnlineTeamMembers(team.getId()).stream())
    ...
    public List<GameTeam> getInitialTeams() {
        if (SettingsManager.getInstance().teamGame().get() == TeamStatus.TEAM_GAME) {
            return Bukkit.getOnlinePlayers().stream()
                .map(TeamService.getInstance()::getPlayerTeam)
                .filter(Objects::nonNull)
                .filter(team -> !team.getId().equals("spectators"))
                .distinct()
                .collect(Collectors.toList());
        } else {
            return ListUtils.of();
    ...
    public double getWorldBorderSize() {
        return WorldSyncService.getInstance().getWorldBorderSize();
    }
    ...
    public void handleGameOver() {
        ...
        WorldSyncService.getInstance().stopWorldBorder();
        this.timer.stop();
